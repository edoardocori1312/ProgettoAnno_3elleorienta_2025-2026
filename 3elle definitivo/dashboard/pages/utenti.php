...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['azione']) && $_POST['azione'] === 'aggiungi') {
    ...
    $tipo     = in_array($_POST['tipo'] ?? '', ['ADMIN', 'SCOLASTICO']) ? $_POST['tipo'] : 'SCOLASTICO';

    if ($username && $email && $password) {
        $hash = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $conn->prepare("INSERT INTO Utenti (username, hash_password, email,  tipo, stato) VALUES (?, ?, ?, ?, 'ATTIVO')");
        $stmt->bind_param('ssss', $username, $hash, $email, $tipo);
        if ($stmt->execute()) {
            $_SESSION['flash_msg']  = 'Utente creato con successo.';
            $_SESSION['flash_type'] = 'success';
        } else {
            $_SESSION['flash_msg']  = 'Errore durante la creazione (email o username già esistenti?).';
            $_SESSION['flash_type'] = 'danger';
        }
        $stmt->close();
    } else {
        $_SESSION['flash_msg']  = 'Compila tutti i campi obbligatori.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['usEliminato'])) {
    ...
    $ok = $stmt->execute();
    $_SESSION['flash_msg']  = $ok ? 'Utente eliminato.' : 'Errore eliminazione.';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['usDisattivato'])) {
    ...
    if ($riga) {
        $nuovo = ($riga['stato'] === 'ATTIVO') ? 'BLOCCATO' : 'ATTIVO';
...
// ── Ricerca (filtro PHP) ────────────────────────────────────────────────────
$ricerca        = strtolower(trim($_POST['parolaChiave'] ?? ''));
