<?php 
   ...
   function modificaEventoScolastico($idUtente, $idEvento, $titolo, $descrizione, $ora_inizio, $ora_fine, $visibile, $prenotabile, $descrizioneBreve, $foto)
        ...
        $stmt = null;
            ...
            else
            {
                ...
                else
                {
					...
					{
						$stmt->bind_param("ssssiissssii", $titolo, $descrizione, $ora_inizio, $ora_fine, $visibile, $prenotabile, $via_P, $n_civico_P, $punto, $descrizioneBreve, $id_citta, $idEvento);
						if(!$stmt->execute())
						{
							$risultato->errore = "errore nell'esecuzione dello statement";
						}
    ...
    function controllaData($data_inizio, $data_fine)
    {
        ...
        else if($data_fine <= $data_inizio || $data_inizio==null)
        {
            return false; 
        }
		else{
    ...
    function recuperaIdFoto($idEvento)
    {
        $risultato = new risultato();
    ...
    function disegnaTabella($riga)
    {
        ...
        $resScuola = recuperaNomeScuola($riga['cod_scuola']);
        if($resScuola->isSuccess() && $resScuola->result)
        {
