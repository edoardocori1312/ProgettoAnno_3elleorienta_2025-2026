...
$filtro_attivo = (isset($_GET['lat']) && isset($_GET['lng'])) || isset($_GET['data_da']) || isset($_GET['data_a']) || (isset($_GET['cerca']) && $_GET['cerca'] !== '');

// Modalità fragment: restituisce solo card + paginazione (usata dal fetch in main.js)
if (isset($_GET['fragment'])) {
    header('Content-Type: text/html; charset=UTF-8');
    if (!empty($no_events_msg)):
?><p class="no-events-msg" style="grid-column:1/-1"><i class="bi bi-calendar-x"></i> <?= $no_events_msg ?></p>
<?php else:
    foreach ($events as $evt):
        $img            = $evt['image_path'] ? $evt['image_path'] : 'https://3elleorienta.sviluppo.host/pictures/nofoto.jpg';
...
<div class="hero-banner">
    <div class="header">
        <h1>Eventi</h1>
        <p class="paragraph">Esplora gli eventi disponibili e partecipa a quello che più ti attira</p>
    </div>
</div>
    ...
    <div class="three-col-layout">
        ...
        <div class="col-center">
            ...
            <div id="events_section" class="cards-wrapper" aria-live="polite">
                ...
                <!-- Paginazione (dentro events_section, sovrascritta dal fetch) -->
                ...
                <p class="pagination-info" style="grid-column:1/-1">
        ...
        <div class="col-right">
            ...
            <div class="date-filter-control">
                <label><i class="bi bi-calendar3"></i> Filtra per periodo</label>
                <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                    <div style="flex:1; min-width:100px;">
                        <small style="display:block; margin-bottom:3px; color:#6c757d;">Dal</small>
                        <input id="filtro_data_da" type="date"
                               value="<?= htmlspecialchars($_GET['data_da'] ?? '') ?>"
                               aria-label="Data inizio periodo">
                    </div>
                    <div style="flex:1; min-width:100px;">
                        <small style="display:block; margin-bottom:3px; color:#6c757d;">Al</small>
                        <input id="filtro_data_a" type="date"
                               value="<?= htmlspecialchars($_GET['data_a'] ?? '') ?>"
                               aria-label="Data fine periodo">
                    </div>
                </div>
                <?php if (isset($_GET['data_da']) || isset($_GET['data_a'])): ?>
