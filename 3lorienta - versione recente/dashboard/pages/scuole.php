...
$cod_scuola_utente = $_SESSION['cod_scuola'] ?? null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'inserisci') {
    ...
    // Lo SCOLASTICO non può inserire nuove scuole
    if ($is_scolastico) {
...
if (($_GET['action'] ?? '') === 'elimina' && !empty($_GET['cod'])) {
    ...
    // Lo SCOLASTICO non può eliminare alcuna scuola
    if ($is_scolastico) {
...
// Lo SCOLASTICO vede solo la propria scuola, ignora filtri
if ($is_scolastico) {
    $sql_lista  .= " AND s.COD_meccanografico = ?";
    $params[]    = $cod_scuola_utente;
    $param_types .= "s";
} else {
    if ($filtro_citta > 0) {
...
<div class="content-grid">
                    ...
                    <table class="table table-hover table-scuole mb-0">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <?php
                                $può_modificare = $is_admin || ($is_scolastico && $row['COD_meccanografico'] === $cod_scuola_utente);
                                ?>
                                <?php if ($può_modificare): ?>
                                <a href="index.php?page=scuola_modifica&cod=<?= urlencode($row['COD_meccanografico']) ?>"
                                   class="btn btn-outline-primary btn-sm me-1" title="Modifica">
    ...
    <div class="grid-full">
        <div class="card-panel">
            ...
            <form method="POST" action="index.php?page=scuole" enctype="multipart/form-data">
    ...
    <?php endif; // fine form inserimento ADMIN ?>
</div>
